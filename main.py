...
SYSTEM_PROMPT = (
    ...
    "to ask about. Keep formatting simple and mobile-friendly: short "
    "paragraphs, plain text, minimal markdown, since this is read on a phone "
    "inside Telegram.\n\n"
...
# --- Feature: user sends a voice note, bot transcribes and responds ---
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = update.effective_user.username or update.effective_user.first_name
    logger.info("VOICE from %s (id=%s), duration=%s", username, user_id, voice.duration)
    ...
    try:
        ...
        response = model.generate_content(
            [
                ...
                "Keep your response focused and exam-relevant for a parasitology/entomology student.",
                {"mime_type": "audio/ogg", "data": bytes(audio_bytes)},
            ]
        )
        reply_text = response.text
    except Exception:
        logger.exception("Voice transcription/analysis failed")
        await update.message.reply_text(
            "Sorry, I couldn't process that voice note. Try again, or send a text message instead."
        )
        return
