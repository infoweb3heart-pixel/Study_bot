...
# --- Persistent bottom menu (like the "Buy Voucher / My Purchases" style menu) ---
MENU_ASK = "❓ Ask a Question"
MENU_TOPIC = "📚 Topic Deep Dive"
MENU_QUIZ = "🧠 Quiz Me"
MENU_FLASHCARD = "🗂️ Flashcards"
MENU_POLLQUIZ = "📊 Poll Quiz"
...
MAIN_MENU_KEYBOARD = ReplyKeyboardMarkup(
    [
        [MENU_ASK, MENU_TOPIC],
        [MENU_QUIZ, MENU_FLASHCARD],
        [MENU_POLLQUIZ, MENU_CLEAR],
        [MENU_HELP],
    ],
...
# Prompts shown after a menu tap, before we know the topic. Keyed by the
# same action names used in handle_message's routing below.
MENU_TOPIC_PROMPTS = {
    "topic": "What topic do you want a full explanation of? e.g. *Cestoda life cycles*",
    "quiz": "What should I quiz you on? Add a number for question count, e.g. "
    "*Cestoda 10*. Or just say *surprise me*.",
    "flashcard": "What topic should the flashcards cover? You can add a count, "
    "e.g. *Trematodes 12*.",
    "pollquiz": "What topic for the poll quiz? Add a count if you want, e.g. "
    "*Vectors 5*.",
}

# Tracks a user who just tapped a menu button and needs to send the topic
# as their next message, e.g. {123: "quiz"}.
_pending_menu_action: dict[int, str] = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "**Parasitopia** — your Parasitology & Entomology study bot\n"
        "Covers Helminthology, Protozoology, Medical Entomology, Vector "
        "Biology, Entomological Techniques, and more.\n"
        "\n"
        "**Study commands**\n"
        "- /ask `<question>` — tutor-style answer to anything\n"
        "- /topic `<topic>` — full exam-focused explanation\n"
        "- /quiz `<topic> [count]` — interactive quiz, one question at a time\n"
        "- /flashcard `<topic> [count]` — tappable flashcards\n"
        "- /pollquiz `<topic> [count]` — MCQs as native Telegram polls\n"
        "\n"
        "_Example: /quiz Cestoda 10_\n"
        "\n"
        "**Other**\n"
        "- /clear — reset our conversation history\n"
        "\n"
        "**No command needed**\n"
        "- Just type a message — I'll answer directly\n"
        "- Send a photo of a diagram — I'll explain it\n"
        "- Send a PDF, .txt, or .pptx — I'll summarize it (add a caption to "
        "tell me what to do with it)\n"
        "- Ask to \"see a picture of X\" — I'll find a real one\n"
        "\n"
        "_Tip: use the buttons below — no need to remember the slash "
        "commands._"
    )
    await send_formatted(update, text, reply_markup=MAIN_MENU_KEYBOARD)
...
async def handle_menu_tap(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    """If `text` is one of the bottom-menu button labels, handle it and
    return True. Buttons that need a topic just record what the user wants
    and ask for it — the next plain-text message they send is then treated
    as that topic (see the pending-action check in handle_message)."""
    user_id = update.effective_user.id

    if text == MENU_ASK:
        _pending_menu_action[user_id] = "ask"
    ...
    action = {
        ...
        MENU_POLLQUIZ: "pollquiz",
    }.get(text)

    if action:
        _pending_menu_action[user_id] = action
        await update.message.reply_text(
            telegramify_markdown.markdownify(MENU_TOPIC_PROMPTS[action]),
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await handle_menu_tap(update, context, text):
        return

    pending = _pending_menu_action.pop(user_id, None)
    if pending:
        if text.strip().lower() in ("surprise me", "surprise", "anything", "any"):
            text = ""
        context.args = text.split()

        if pending == "ask":
            await handle_ai_reply(update, text or "Ask me something worth knowing from the syllabus.")
        elif pending == "topic":
            if text:
                await topic_deep_dive(update, context)
            else:
                await update.message.reply_text("Give me a topic name and try again.")
        elif pending == "quiz":
            await quiz(update, context)
        elif pending == "flashcard":
            await flashcard(update, context)
        elif pending == "pollquiz":
            await pollquiz(update, context)
        return
